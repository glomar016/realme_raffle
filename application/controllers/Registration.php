...
class Registration extends CI_Controller {
	...
	{
		if($this->input->post('storeName') == "Others"){
			$newStoreName = $this->input->post('otherStoreName');
		}
		else{
			$newStoreName = $this->input->post('storeName');
		}
					...
					<body style="background-color: #ea273d; max-width:100%; margin:auto; padding:10px">  
						<div style="margin:50px; background-color:#222a5b; border-radius: 25px;">
							<div style="text-align: center; margin:0px; padding:25px;">
								<h2 style="color:white; font-family:arela Round, Trebuchet MS, Helvetica, sans-serif; color:#ffc003">Hi <font color="white"> '.$result['first_name'].'!</font></h2> 
								<h4 style="color:white; font-family:arela Round, Trebuchet MS, Helvetica, sans-serif; color:#ffc003" >Click this link to get your verification code: <br> 
								'.base_url().'registration/get_code/'.$id.'</h4>
							</div>
						</div>
					</body> 
				</html>
